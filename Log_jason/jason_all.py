...
LOGIC_FILE = r"C:\Users\wei\Documents\UA_APP0.1\Log_jason\TKG_final_Cards_Logic_0.json"
#
# === LLM API 配置 ===
...
API_KEYS = [
    ...
    "AIzaSyAqj1UNDf64T1c5P-w7nz7lsqCpjO4hlpQ",
    "AIzaSyCh9lpP9LgUxDj_EtQENxpuPEqgZ1RBNQQ", # api key 專題

    "AIzaSyBEWrXaF2HPebA1UcdC0ZW3gMl3SFTI29g",
    "AIzaSyB0PNYcRy_gSM28PnV-kx5O_uhuzDfNiko", # 李元1

    "AIzaSyBdWaQahXfm2pEw2IDgv-wHpmRfURqcA9U"
] 
current_key_idx = 0
...
6. 嚴禁發明幽靈變數：遇到「同名卡」時，請直接從文本中提取實際卡名（如 "name": "亞門 鋼太朗"），或使用 "self_card"。絕對禁止輸出類似 "$self_card_name" 這種未定義的變數名稱！

【⚠️ 領域與位置絕對對應表 (Location Mapping)】
遇到卡牌文本描述位置時，請【嚴格】使用以下對應的英文單字，絕對禁止混用或自創：
1. 場外 (Discard Pile) ＝ "outside"
2. 除外區 (Removed Area/Exile) ＝ "removed_area"
...
3. 異色能源判定：遇到「除外區有 5 種不同顏色的能源」這類要求時，請使用 `{"eval_type": "unique_energy_requirement"}`。
4. 動態減費算式：在使用 `"dynamic"` 減費時，`"multiplier"` (倍率) 一律使用正整數（如 1），因為積木 "cost_reduction" 本身就已經代表了「減少」。
5. AP 恢復：遇到「恢復 AP」或「將 AP 卡激活」的效果，【一律】使用專用積木 `{"type": "activate_ap", "amount": 數字}`。
...
4. 數值與關鍵字積木 (Stats & Keywords):
   ...
   - `{"type": "gain_keyword", "target": {目標選取器}, "keywords": {"impact": 數字, "impact_nullify": true, "damage": 1, "step": true, "cannot_stand": true, "snipe": true, "twice_attack": true, "twice_block": true, "cannot_be_selected": true, "must_block": true, "cannot_be_blocked_by": {"bp_threshold": 4000}}, "duration": "turn"|"continuous"}`
...
class UAParser:
        ...
        for attempt in range(max_retries):
            try:
                ...
                response = client.models.generate_content(
                    ...
                    contents=f"這是一張【{type}】。\n請將以下卡牌效果轉換為符合規則的 JSON：\n{text}",
    ...
    @classmethod
    def extract_advanced_logic(cls, raw_text):
        ...
        # ==========================================
        try:
            llm_result = cls._call_llm_parser(text) 
        except Exception as e:
...
def run_split():
    ...
    for idx, card in enumerate(raw_list[0:167]): # 測試階段維持 0:10
        ...
        combined_main_text = "\n".join([e.get("text", "") for e in card.get("main_effects", [])]).strip()
        ...
        if not skip_llm:
            try:
                parsed_logic_tags = UAParser.extract_advanced_logic(combined_main_text)
            except Exception as e:
                print(f"  ❌ 解析發生錯誤: {e}")
