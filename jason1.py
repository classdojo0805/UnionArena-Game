...
LOGIC_FILE = os.path.join(BASE_DIR, "TKG_final_Cards_Logic_0.json")
#
# === LLM API 配置 ===
api_keys_1 = os.getenv("API_KEYS_1").split(",")
api_keys_2 = os.getenv("API_KEYS_2").split(",")

API_KEYS = api_keys_1 + api_keys_2
current_key_idx = 0
...
在生成積木前，你必須先在 `_todolist` 中完成品管打卡。若遇到目前提供的【字典】中完全找不到對應函數或積木的情況，你必須在第 4 步明確標註。

{
  "_todolist": {
    "1_check_card_type_and_timing": "確認卡片類型，並宣告允許使用的觸發時機。",
    "2_scan_special_rules": "掃描文本是否有『作為代價』、『全部AP』等特殊限制。",
    "3_confirm_effect_count": "確認任務面板是否有劃分【獨立效果】標籤。",
    "4_function_mapping_check": "逐一核對字典積木。若無法對應，此處填寫：『找不到對應函數名稱：{無法翻譯的原文}』；若全數對應則寫：『字典匹配成功』。"
  },
  "effect_blocks": {
    "觸發時機": [
      // 匹配成功的積木，若匹配失敗則該效果留空或使用 custom 積木
    ]
  }
}

【🚨 必須嚴格遵守的絕對限制 (防呆底線)】
1. 誠實原則：若字典中沒有對應的 type 或功能，絕對禁止自創枚舉值 (Enum)！必須在 `_todolist` 報錯，並在 `effect_blocks` 中將該動作標記為 `{"type": "custom", "description": "找不到對應函數名稱"}`。
2. 絕對禁止註解：輸出的 JSON 內嚴禁出現 `//` 或 `/*`。
3. 手牌 vs 場上 絕對隔離：
   - 「手牌」放置到場外/移除區 ＝ `discard`。
   - 「場上角色」放置到場外/移除區 ＝ `retire`。
4. 拒絕無腦降級：除非字典真的沒有，否則優先使用 `conditional_branch` 或 `alternative_cost`。
5. 嚴禁發明變數與枚舉值：遇到卡名請直接寫出或用 "self_card"。字典裡規定的字串絕對不允許縮寫！
...
`{"player": "self"|"opponent"|"both", "location": "front_line"|"energy_line"|"hand"|"deck"|"outside"|"removed_area"|"field"|"raid_source", "amount": 數字, "modifier": "up_to"|"exact", "filters": {"trait": "...", "name": "...", "color": "...", "state": "active"|"rest", "card_type": "...", "max_bp": 數字, "min_bp": 數字, "max_energy": 數字, "ap_consumption": 數字, "history": "played_this_turn", "other": true/false, "has_underneath_card": true/false, "underneath_face_down": true/false, "unique_names": true/false, "any_of": []}}`
- 重點：第二個起針對同目標的動作，target 必須寫 `"selected_target"`。

【🧱 效果區塊結構 (Effect Block Schema)】
每個區塊包含： "optional" (boolean), "limit" (Object), "conditions" (Array), "costs" (Array), "actions" (Array)。
"""
...
def run_split():
    ...
    for idx, card in enumerate(raw_list[0:41]): # 測試階段維持 0:10
